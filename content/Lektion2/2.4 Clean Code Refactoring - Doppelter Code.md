...
Ein Problem ist, dass die load Methoden unterschiedliche Rückgabetypen haben. Da wir nicht wissen können, was für eine Ressource wir laden, können wir hier nur einen Platzhalter angeben: Ein Generic. Diese werden in der Regel durch einen großen Buchstaben angeben und wir könnten hier R wählen (R wie "Rückgabe").

Mit dieser Überlegung können wir uns schon einmal den Rahmen der Klasse / der Methode bauen: Ein universellen ResourceServer, der bei loadResource ein Element vom Typ R zurück gibt:
...
Bei den Klassen war ein weiterer Unterschied, dass für den Prefix und den Typ der Ressource jeweils zwei Konstanten bereit gestellt wurden. Diese Konstanten benötigen wir dann auch in unserer neuen Klasse. Jedoch können wir hier die Werte noch nicht wissen. Daher müssen die Werte dieser Konstanten nicht über Literale gesetzt werden sondern durch den Konstruktor, so dass es keine Konstanten im bisherigen Sinne sind sondern finale Instanzvariablen.
...
- Bei der Version mit der Function als zusätzlicher Parameter hat man den Vorteil, dass der Name der Methode bei der Implementation frei wählbar ist. Der Name kann also genau aussagen, was dort in der Method gemacht wird. Dadurch sind die ablgeleiteten Klassen einfacher zu lesen. Der Nachteil ist aber dann im ResourceService: hier wird dann nur noch function.apply aufgerufen, was nicht ganz so viel aussagt. Hund in der Function, die übergeben wird, muss eine Exception behandelt werden.

- Bei der zweiten Version mit dem abstrakten Methode ist der Name der Methode vorgegeben: Die Methode muss createResource heissen. Damit kann man nicht mehr den Namen so wählen, wie er aus Sicht der abgeleiteten Klasse am besten wäre. Und da wird eine IOException erlauben können, muss hier keine zusätzliche Behandlung in der Methode selbst erfolgen.
